...
class R2D2(Node):
    ...
    def is_new_point(self,ox_val, oy_val, mapped_points):
        """
        Verifica se o ponto (ox_val, oy_val) já foi mapeado dentro do limiar de distância.
        """
        for (px, py) in mapped_points:
            dist = np.sqrt((ox_val - px) ** 2 + (oy_val - py) ** 2)
            if dist < dist_threshold:
                # O ponto está muito próximo de um ponto já mapeado, então ignorar
                return False
        return True
    ...
    def mapa_global(self,dist,yaw,map1,ax2,fig):
        ...
        # Filtra os pontos para evitar remapeamento incorreto
        #global mapped_points
        #filtered_ox = []
        #filtered_oy = []

        #for ox_val, oy_val in zip(ox, oy):
        #    if self.is_new_point(ox_val, oy_val, mapped_points):
        #    # Se o ponto é novo, adiciona ao mapa e à lista de pontos filtrados
        #        mapped_points.add((ox_val, oy_val))  # Adiciona ao conjunto de pontos mapeados
        #        filtered_ox.append(ox_val)
        #        filtered_oy.append(oy_val)

        # Converte listas de volta para arrays do NumPy para retorno
        #filtered_ox = np.array(filtered_ox)
        #filtered_oy = np.array(filtered_oy)

        #xyreso = 0.125 #resolução do grid do mapa em metros
        #pmap, minx, maxx, miny, maxy, xyreso = lg.generate_ray_casting_grid_map(oy, ox, xyreso, False)
        #xyres = np.array(pmap).shape
        ## Configurando o gráfico para usar o subplot ax2
        #ax2.imshow(pmap)  # Salva o objeto de imagem retornado por imshow
        #ax2.set_xticks(np.arange(-.5, xyres[1], 1), minor=True)
        #ax2.set_yticks(np.arange(-.5, xyres[0], 1), minor=True)
        #ax2.grid(True, which="minor", color="w", linewidth=.6, alpha=0.5)

        # Adiciona barra de cores ao subplot ax2
        #fig.colorbar(ax2.imshow(pmap, cmap="PiYG_r"), ax=ax2)
        #------------------------------------------------------------------------------
    ...
    def run(self):
            ...
            while(rclpy.ok):
                ...
                #self.flood_fill((int(pos_x*Escala),int(pos_y*Escala)),map1)

                #Plota o gráfico global
                #ax2.axis("equal")

                #------------------------------------------------------------------------------
                #Ajustes para o plot de pontos vermelhos:
                #ax2.plot(ox,oy, marker='o', linestyle='None', markersize=1,color='red')
                #------------------------------------------------------------------------------                
